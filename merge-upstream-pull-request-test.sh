...
readonly BASE_PULL_URL="https://github.com/Yorizuka/test-upstream/pulls"
...
type curl >/dev/null 2>&1 || { echo >&2 "Error: This script requires curl, please ensure curl is installed and exists in the current PATH"; exit 3; }

# Ensure jq exists and is available in the current context
type jq >/dev/null 2>&1 || { echo >&2 "Error: This script requires jq, please ensure jq is installed and exists in the current PATH"; exit 4; }
...
if ! git remote | grep tgstation > /dev/null; then
   git remote add tgstation https://github.com/Yorizuka/test-upstream.git
fi
